...
#This file implements the scikit version of Multi Layer Perceptronclassification for our data set. It calls the GetData function of the BalanceData module to obtain the training features and labels and testing features, labels and keys. Then it does a grid search on the testing data to obtain optimum paramters for the training data using cross validation. It then calls the fit method on the Multi Layer Perceptron to fit the model to the training features and labels available. The test features are tested on the fitted model to predict the labels for test features which are later used to obtain the test accuracy and confusion matrix. This module also calls the predict_proba method of the model to get per track, the probability of it being in a particular class. This predicted probability per track is stored in teh file NeuralNetsOut which shall be later be used by the combine method.  
